            ...
            <table class="table table-borderless table-striped table-sm">
                ...
                @foreach($invoice->items as $index=>$item)
                    <tr>
                        ...
                        <td class="text-right">{{$item->rate? $item->rate : '-'}}</td>
                        <td class="text-right">{{$item->amount? $item->amount : '-'}}</td>
                    </tr>
                @endforeach
        ...
        <tr>
            ...
            <td style="width: 40%; vertical-align: top" class="text-right">
                <table class="table table-sm float-right">
                    <tr class="text-right">
                        <th>Total:</th>
                        <td>{{'BDT '}}{{$invoice->sub_total}}</td>
                    </tr>
                    @if($invoice->tax)
                        <tr class="text-right">
                            <th class="border-0">TAX:</th>
                            <td class="border-0">{{'BDT '}}{{(($invoice->tax/100)*$invoice->sub_total)}}
                            </td>
                        </tr>
                    @endif
                    @if($invoice->vat)
                        <tr class="text-right">
                            ...
                            <td class="border-0">{{'BDT '}}{{(($invoice->vat/100)*$invoice->sub_total)}}
                            </td>
                        </tr>
                    @endif
                    @if(($invoice->discount || $invoice->advance_paid) && ($invoice->vat || $invoice->tax))
                        <tr class="text-right">
                            <th>Net Amount:</th>
                            <td>{{'BDT '}}{{$invoice->net_amount}}</td>
                        </tr>
                    @endif
                    @if($invoice->discount)
                        <tr class="text-right">
                            <th class="border-0">Discount:</th>
                            <td class="border-0">{{'BDT '}}{{$invoice->discount}}</td>
                        </tr>
                    @endif
                    @if($invoice->advance_paid)
                        <tr class="text-right">
                            <th class="border-0">Advance Paid:</th>
                            <td class="border-0">{{'BDT '}}{{$invoice->advance_paid}}</td>
                        </tr>
                    @endif
                    <tr class="text-right">
                        <th>Payable:</th>
                        <td>{{'BDT '}}{{($invoice->net_amount-$invoice->discount-$invoice->advance_paid)}}
                        </td>
                    </tr>
                </table>
            </td>
